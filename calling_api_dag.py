...
def get_testing_increase(state, ti):
    ...
    ti.xcom_push(key='compartment_id',value='wq89473897489327489327489.ocid.skdfjhdjskfhk')
...
def analyze_testing_increases(state, ti):
    ...
    compartment_id=ti.xcom_pull(key='compartment_id', task_ids='get_testing_increase_data_{0}'.format(state))
...
with DAG('nikesh_ex_bash',
    ...
    opr_analyze_testing_data = PythonOperator(
        task_id = 'get_compartment_details',
        python_callable=analyze_testing_increases,
				op_kwargs={'state':state}
    )

    run_this = BashOperator(
    task_id='check_nikesh',
    bash_command='ls -l',
)

    opr_get_covid_data >> opr_analyze_testing_data >> run_this
